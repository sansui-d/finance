<template>
	<view>
		<uni-nav-bar leftIcon="back" @clickLeft="myback" title="我的保险"></uni-nav-bar>
		<view class="pay">
			<text>保险金额：{{ all }}</text>
			<view class="top">
				<view class="topPay"><text>姓名</text><text>{{ user.name }}</text></view>
				<view class="topIncome"><text>账户</text><text>{{ account.actName }}</text></view>
				<view class="topAll"><text>保险数</text><text>{{ debt.length }}</text></view>
			</view>
			<view class="info">
				<view class="title">
					<view><text>保险项</text></view>
				</view>
