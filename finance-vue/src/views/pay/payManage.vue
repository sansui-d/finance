<template>
  ...
  >
    ...
    <el-table :data="tableData" style="width: 100%" highlight-current-row fit>
      <el-table-column fixed prop="outTime" label="时间"> </el-table-column>
      <el-table-column fixed prop="actName" label="账户"> </el-table-column>
      <el-table-column fixed prop="name" label="姓名"> </el-table-column>
      <el-table-column fixed prop="money" label="支出"> </el-table-column>
...
<script>
import payAdd from "../pay/payAdd";
import payDetail from "../pay/payDetail";
    ...
    handleDelete(row) {
      this.$confirm("确定删除此支出?", "提示", {
        ...
        .then(() => {
          ...
          this.$axios
            .get("/api/pay/deletePay", { params: { payId } })
            .then((res) => {
              if (res.data == 200) {
                this.init();
                this.$message({
