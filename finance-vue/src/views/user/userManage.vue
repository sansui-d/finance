<template>
  ...
  >
    ...
    <el-table :data="tableData" style="width: 100%" highlight-current-row fit>
      <el-table-column fixed prop="time" label="日期"> </el-table-column>
      <el-table-column fixed prop="name" label="姓名"> </el-table-column>
      <el-table-column fixed prop="loginId" label="账号"> </el-table-column>
      <el-table-column fixed prop="loginPass" label="密码"> </el-table-column>
      <el-table-column fixed prop="sex" label="性别"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    init(
      ...
      const loginId = this.formInline.loginId;
      this.query = query;
      const pageNum = this.query.pageNum;
      const pageSize = this.query.pageSize;
      this.$axios
        .get("/api/user/selectUser", {
          params: { name, loginId, pageNum, pageSize },
        })
        .then((res) => {
          ...
          for (let i in this.tableData) {
            this.tableData[i].loginPass = this.passHide(
              this.tableData[i].loginPass.length
            );
          }
    ...
    handleDelete(row) {
      this.$confirm("确定删除此用户?", "提示", {
        ...
        .then(() => {
          ...
          this.$axios
            .get("/api/user/deleteUser", { params: { userId } })
            .then((res) => {
              if (res.data == 200) {
                this.init();
                this.$message({
    ...
    select() {
      ...
      if (formInline.name == "" && formInline.loginId == "") {
        this.isSelect = false;
      } else {
        this.isSelect = true;
